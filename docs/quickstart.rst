...
安装Python 3.4+和 ``pip3``, JDK 8 和git 1.9+。然后执行下面的命令，如果需要的话在前面加上 ``sudo``::
...
这个操作会下载Elasticsearch 5.0.0，然后执行Rally的默认track。执行完成后，会在命令行产生一个总结报告::
...
现在你可以查看 :doc:`怎样执行基准测试 </race>` 或者 :doc:`怎样定义你自己的基准测试 </adding_tracks>`
